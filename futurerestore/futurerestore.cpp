...
void futurerestore::enterPwnRecovery(plist_t build_identity, std::string bootargs){
    ...
    if(!_noIBSS) {
        ...
        if (_setibss) info("NOTE: Using custom iBSS: %s\n", _iBSSPath);
        mutex_lock(&_client->device_event_mutex);
    ...
    if((_client->device->chip_id >= 0x7000 && _client->device->chip_id <= 0x8004) || (_client->device->chip_id >= 0x8900 && _client->device->chip_id <= 0x8965)) {
        ...
        if (_client->build_major > 8) {
            ...
            /* send iBEC */
            info("Sending %s (%lu bytes)...\n", "iBEC", iBEC.second);
            if(_setibec) info("NOTE: Using custom iBEC: %s\n", _iBECPath);
            mutex_lock(&_client->device_event_mutex);
...
void futurerestore::setiBSSPath(const char *iBSSPath) {
    _setibss = true;
    FILE *fbb = NULL;

    retassure(fbb = fopen(iBSSPath, "rb"), "failed to read iBSS img4");
    _iBSSPath = iBSSPath;
    fclose(fbb);
}
...
void futurerestore::setRamdiskPath(const char *ramdiskPath) {
    ...
    retassure(fbb = fopen(ramdiskPath, "rb"), "failed to read ramdisk im4p");
...
void futurerestore::setKernelPath(const char *kernelPath) {
    ...
    retassure(fbb = fopen(kernelPath, "rb"), "failed to read kernel im4p");
